...
describe(`Create new logbook as a logbook designer`, () => {
	let browser, page
	let newWindow
	...
	test(`Logbook-designer overview`, async () => {
		await page.click(`.bttn.green`)
		expect(page.url()).toBe(
			'https://localhost:3001/logbook-designer/new-logbook/general'
		)
	})
	...
	test(`Create logbook: Happy path - general`, async () => {
		...
		expect(page.url()).toBe(
			'https://localhost:3001/logbook-designer/new-logbook/columns'
		)
	})
	...
	test(`Create logbook: Happy path - columns`, async () => {
		...
		await page.click(`.next button`)
		expect(page.url()).toBe(
			'https://localhost:3001/logbook-designer/new-logbook/goals'
		)
	})
	...
	test(`Create logbook: Happy path - goals`, async () => {
		...
		await page.click(`.next button`)
		expect(page.url()).toBe(
			'https://localhost:3001/logbook-designer/new-logbook/overview'
		)
	})
	...
	test(`Create logbook: Happy path - overview`, async () => {
		...
		expect(page.url()).toBe(
			'https://localhost:3001/logbook-designer/new-logbook/done'
		)
	})
})
